...
SD_Error SD_Init(void)
{
	...
	if (errorstatus == SD_OK)errorstatus = SD_SelectDeselect((uint32_t)(SDCardInfo.RCA << 16));//选中SD卡   
...
SD_Error SD_InitializeCards(void)
{
	...
	if (SDIO_MULTIMEDIA_CARD == CardType)
	{
		...
		SDIO_CmdInitStructure.SDIO_Argument = (uint32_t)(rca << 16);//发送CMD3,短响应 
		SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_SET_REL_ADDR;	//cmd3
		SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short; //r6
		SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
		SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
...
SD_Error SD_WriteBlock(uint8_t* buf, long long addr, uint16_t blksize)
{
	...
	SDIO_CmdInitStructure.SDIO_Argument = (uint32_t)RCA << 16;//发送CMD13,查询卡的状态,短响应 	
	...
	else if (DeviceMode == SD_DMA_MODE)
	{
		...
		SD_DMA_Config((uint32_t*)buf, blksize, DMA_DIR_MemoryToPeripheral);				//SDIO DMA配置
		SDIO->DCTRL |= 1 << 3;								//SDIO DMA使能.  
		while (((DMA2->LISR & (1 << 27)) == RESET) && timeout)timeout--;//等待传输完成 
		if (timeout == 0)
		{
...
SD_Error SD_WriteMultiBlocks(uint8_t* buf, long long addr, uint16_t blksize, uint32_t nblks)
{
	...
	if (nblks > 1)
	{
		...
		else if (DeviceMode == SD_DMA_MODE)
		{
			...
			TransferEnd = 0;				//传输结束标置位，在中断服务置1
			SDIO->MASK |= (1 << 1) | (1 << 3) | (1 << 8) | (1 << 4) | (1 << 9);	//配置产生数据接收完成中断
			SD_DMA_Config((uint32_t*)buf, nblks * blksize, DMA_DIR_MemoryToPeripheral);		//SDIO DMA配置
			SDIO->DCTRL |= 1 << 3;								//SDIO DMA使能. 
			timeout = SDIO_DATATIMEOUT;
			while (((DMA2->LISR & (1 << 27)) == RESET) && timeout)timeout--;//等待传输完成 
			if (timeout == 0)	 								//超时
...
void Init_SDCARD(void)
{
	if (SD_Init() != 0)
	{
		p_info("> SD STORAGE_Init error");; 
	}
	else
	{
		uint32_t total, free, timeout = 0;
		p_info("> SD STORAGE_Init Ok");
		exfuns_init();							// 为fatfs相关变量申请内存				 
		f_mount(fs[0],"0:",1); 					// 挂载SD卡 
		while(exf_getfree("0", &total,&free) && (timeout < 10))	// 得到SD卡的总容量和剩余容量
		{
			p_info("SD Card Fatfs Error! retry...");
			timeout++;
			delay_ms(100);
		}
		...
		if(timeout < 10)
		{
			SDCARD_Status = SD_CARD_STATUS_OK;
			p_info("FATFS OK!");	 
			p_info("SD Total Size:%d MB", total>>10);	 
			p_info("SD  Free Size:%d MB", free>>10); 
		}
		else
		{
			SDCARD_Status = SD_CARD_STATUS_ERROR;
		}
	}
}
